...
3. **Recepción de mercadería sin trazabilidad de lote:** los proveedores entregan la mercadería en furgones completos; actualmente no existe forma de rastrear qué mercadería llegó en qué furgón/lote, dificultando la identificación de problemas de calidad o faltantes por entrega.
...
- Gestión de proveedores y órdenes de compra con **número de furgón/lote** para trazabilidad de la mercadería recibida (sin gestionar logística externa ni aduanas — únicamente el registro del ingreso a bodega), y su detalle de ingreso.
...
### Módulo 2 — Compras, Proveedores y Recepción de Furgones
| ID | Requerimiento |
|---|---|
| RF06 | El sistema debe permitir registrar proveedores con su información de contacto. |
| RF07 | El sistema debe permitir registrar una orden de compra asociada a un proveedor, identificada con su número de furgón/lote, con el detalle de variantes y cantidades adquiridas. |
| RF08 | El sistema debe registrar el ingreso de mercadería a bodega proveniente de un furgón y actualizar automáticamente el stock de las variantes correspondientes. |
| RF09 | El sistema debe permitir consultar el historial de compras filtrado por proveedor o por número de furgón/lote. |
...
| RD05 | Toda compra debe estar asociada a un proveedor, a un empleado responsable, y debe registrar obligatoriamente su número de furgón/lote. |
...
| RD11 | El precio/costo unitario y la cantidad en el detalle de compras/pedidos deben ser valores positivos. |
...
La guía del proyecto exige notación Chen (óvalos para atributos, rombos para relaciones, rectángulos para entidades). **Ni Mermaid.js ni PlantUML soportan nativamente notación Chen**; ambas generan diagramas ER en notación Crow's Foot / IE. Por lo tanto, la especificación textual de esta sección es la fuente de verdad del modelo, y el diagrama Chen oficial (editable + PNG/PDF, estándar D3) se elabora en draw.io/Lucidchart a partir de ella, guardado en `/docs/diagramas/` (archivo `pavanamoon_ER_chen.drawio`, ya actualizado con los cambios de esta versión).
...
6. **CATEGORIA** (categoria_id **PK**, nombre_categoria {Niño|Juvenil|Adulto}, descripcion)
7. **PRODUCTO** (producto_id **PK**, categoria_id **FK**, modelo, tipo_suela {Hule|Flex|TPU}, descripcion, precio_venta, fecha_registro) — *sin campo `marca`: Pavanamoon es marca propia, todos los modelos pertenecen a la misma marca, por lo que ese atributo sería redundante.*
...
11. **COMPRA** (compra_id **PK**, proveedor_id **FK**, empleado_id **FK**, fecha_compra, numero_furgon_lote, total_compra *{derivado}*, estado)
...
| clasifica_por_edad | CATEGORIA | (1,N) | PRODUCTO | (1,1) |
...
> Nota: `numero_furgon_lote` es un **atributo propio de COMPRA** (no una relación aparte) — permite trazabilidad de lote sin modelar logística externa/aduanas, que está fuera del alcance del sistema.
...
erDiagram
    ...
    CATEGORIA ||--o{ PRODUCTO : clasifica_por_edad
...
| ≥15 restricciones explícitas | Se implementan en DDL (Entrega 2): PKs, FKs, UNIQUE (nit, username, sku), CHECK (stock_actual >= 0, cantidad > 0, tipo_suela IN ('Hule','Flex','TPU'), nombre_categoria IN ('Niño','Juvenil','Adulto'), tipo_movimiento IN (...)), NOT NULL (incluyendo numero_furgon_lote) |
...
El equipo utilizó un agente de IA (Claude, Anthropic) como asistente en: (1) la redacción inicial de la propuesta, RF/RD, matriz de trazabilidad y modelo ER conceptual; (2) la corrección de la estructura del repositorio conforme a la Sección 7 de la guía; y (3) esta actualización, que ajusta el modelo de negocio al enfoque real de Pavanamoon como marca propia especializada en calzado de fútbol (eliminación del campo `marca`, incorporación de `tipo_suela`, categorías por edad, y trazabilidad de furgón/lote en compras). El detalle completo del uso, prompts y validación se documenta en `docs/bitacora-ia/BITACORA_IA.md`, conforme al estándar S8/R-Bitácora del proyecto.
